...
var viaplayer = {
	...
	menulist: false,
	playStatus:false,
	api: '//qiatia.cn/tools/viaplayer/api/',
	icon:{
...
getpalylist: function(i) {
		...
		$.post(this.api, {
			...
			viaplayer.viaPic.style.background = 'url(' + data.playlist.coverImgUrl + ')';
	...
	listView:function(i){
		...
		this.viaPic.style.background = 'url(' + this.playlist[i].al.picUrl + ')';
		this.title.innerHTML = '<p>' + this.playlist[i].name + '<span class="via-author"> - ' + this.playlist[i].ar[0].name + '</span></p>';
	},
	listplay: function(i) {
		this.geturl(this.playlist[i].id);
		this.getlyric(this.playlist[i].id);
